...
def convert_action(action):
    ...
    action_converted = {
                ...
                "steering": utils.lmap(action[1], [-1, 1], STEERING_RANGE),
            }
    return action_converted


class HighwayEnv_manual_continuous_tp_carla(AbstractEnv_original):
    ...
    def default_config(cls) -> dict:
        ...
        config.update({
            ...
            "duration": 1000,  # we can double check if it is second
            "ego_spacing": 2,
    ...
    def reset_carla(self):
        ...
        if not self.carla_visualizer:
            self.carla_visualizer = CARLA_viusalizer(image_x=1800 * self.ratio, image_y=900 * self.ratio, animation=False, two_player=True)
    ...
    def _create_vehicles(self) -> None:
        ...
        for others in other_per_controlled:
            ...
            self.neighbor_vehicle = Vehicle_original.create_random(
                self.road,
                speed=25,
                lane_id=self.config["initial_lane_id"],
                spacing=self.config["ego_spacing"] / 8
            )
    ...
    def _info(self, obs, action) -> dict:
        ...
        info = {
            ...
            "action_neighbor": self.action_taken_list
        }
    ...
    def _simulate(self, action=None) -> None:
        ...
        for _ in range(int(self.config["simulation_frequency"] // self.config["policy_frequency"])):
            ...
            self.neighbor_vehicle.act(convert_action(self.action_to_take))
            self.action_taken = self.action_to_take
            self.action_taken_list.append(self.action_taken)
...
register(
    id='highway_manual_continuous_tp_carla-v0',
    entry_point='customized_highway_env.envs:HighwayEnv_manual_continuous_tp_carla',
)
