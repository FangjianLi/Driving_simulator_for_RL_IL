...
from customized_highway_env.envs.common.graphics import EnvViewer_vertical_carla
...
class HighwayEnv_manual_continuous_carla(AbstractEnv_original):
    ...
    def reset_carla(self):
        self.ratio = self.config["ratio"]
        if not self.viewer:
            self.viewer = EnvViewer_vertical_carla(self, self.ratio)
    ...
    def _create_vehicles(self) -> None:
        ...
        for others in other_per_controlled:
            controlled_vehicle = Vehicle_original.create_random(
                self.road,
                speed=25,
    ...
    def _simulate(self, action=None) -> None:
        ...
        for _ in range(int(self.config["simulation_frequency"] // self.config["policy_frequency"])):
            ...
            elif self.config["manual_control"] and self.time % int(
                    self.config["simulation_frequency"] // self.config["policy_frequency"]) == 0:
                self.vehicle.act(convert_action(self.action_to_take))
...
register(
    id='highway_manual_continuous_carla-v0',
    entry_point='customized_highway_env.envs:HighwayEnv_manual_continuous_carla',
)
